...
export default class Meus_Eventos extends React.Component {
  ...
  handleNovoEvento = (nome_usuario, nome_faculdade, nome_curso, selected_heroi, emailUsuario) => {
    ...
    if (chosenDate == '' || titulo == '' || horario == '' || local_link == '') {
      ...
      Toast.show('Evento adicionado com sucesso', Toast.SHORT, Toast.BOTTOM, toastSucesso);
  ...
  handleApagarEvento = (key) => {
    Toast.show('Evento excluido com sucesso', Toast.SHORT, Toast.BOTTOM, toastSucesso);
  ...
  componentDidMount() {
    ...
    const rootRef = firebaseDatabase.ref('Eventos_Privados').orderByChild("usuario_grupo").equalTo(currentUser.uid + this.props.navigation.state.params.nome_grupo_privado);
    ...
    return (
      <View style={styles.container}>
        ...
        <Header androidStatusBarColor="#963BE0" style={{ color: 'black', backgroundColor: '#963BE0', width: '100%' }} >
          ...
          <Button vertical active style={{ backgroundColor: '#6c05da', width: '25%', elevation: 0 }} onPress={() => this.props.navigation.navigate('Meus_Eventos', { nome_grupo_privado: this.props.navigation.state.params.nome_grupo_privado, chave_seguranca: this.props.navigation.state.params.chave_seguranca })}>
