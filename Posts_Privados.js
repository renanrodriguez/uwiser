...
/*Publicação apenas texto */
  ...
  if(this.state.validacao_imagem == 'valido' && this.state.validacao_file==''){
    ...
    if(this.state.validacao_imagem == 'valido' && this.state.validacao_file=='valido'){
      ...
      const filename = `${randomNumber}.${ext}`; // Generate unique name
      this.setState({ uploading: true });
      firebase.storage().ref('images/').putFile(this.state.imageUri).on(
        firebase.storage.TaskEvent.STATE_CHANGED,
        snapshot => {
          let state = {};
...
pickFile = () => { 
  ...
  render() {
    ...
    return (
      <Item body bordered >
      <Button block light style={{color:'black',backgroundColor:'#0000EE',width:'60%'}} onPress={() => this.handleNovaPublicacao(item.nome_usuario,item.nome_faculdade,item.nome_curso,item.selected_heroi,item.emailUsuario)}>
